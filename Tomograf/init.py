...
def save_dicom_data(image): 
    save_dicom(text_name.value,text_image_comment.value,
                                  text_study_date.value,image,text_birthdate.value,text_id.value,text_sex.value,text_weight.value,text_filename.value) 
...
text_weight = widgets.Text(
    value='',
    placeholder='Patient\'s Weight',
    description='',
    disabled=False
)
text_birthdate = widgets.Text(
    value='',
    placeholder="Patient's Birthdate",
    description='',
    disabled=False
)
text_id = widgets.Text(
    value='',
...
def start():
    ...
    im_org = cv2.resize(im_org, (1024,1024))
    gif = []
